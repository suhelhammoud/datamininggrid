...
public class HardDrive extends GridSim {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(HardDrive.class);
	...
	public void body() {
		...
		while (Sim_system.running()) {
			...
			Pair data= (Pair)ev.get_data();
			logger.info(" recieve "+ data +"from "+ ev.get_src()+" at time "+ GridSim.clock());
			if (data.type== Type.READ){
				sim_process(data.size / maxTransferRateRead_);
				super.send(ev.get_src(), 0.0, 0,
						"read");
			}else if (data.type== Type.WRTIE){
				sim_process(data.size / maxTransferRateWrite_);
				super.send(ev.get_src(), 0.0, 0,
						"write");
			}
...
//	private int readSpeed = 60000;
	...
	private double maxTransferRateRead_;

	/** added by suhel in MB/sec */
	private double maxTransferRateWrite_;
	 ...
	 * @throws Exception 
	 */
	public HardDrive(String name, double capacity) throws Exception {
		super(name);
	...
	public void init(){
		...
		avgSeekTime_ = 2; // 9 ms
		maxTransferRateRead_ = 10; // in MB/sec
		maxTransferRateWrite_ =5;
	}
	...
	private void init_old() {
		...
		maxTransferRateRead_ = 133; // in MB/sec
	}
	...
	private double getTransferTime(int fileSize) {
		double result = 0;
		if (fileSize > 0 && capacity_ != 0) {
			result = ((double) fileSize * maxTransferRateRead_) / capacity_;
		}
